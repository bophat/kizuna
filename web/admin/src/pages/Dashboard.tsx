...
export default function Dashboard() {
  ...
  const { stats, loading, startDate, endDate, setStartDate, setEndDate } = useDashboardStats();
          ...
          <div className="mt-8 space-y-3">
            {stats?.revenue_by_category?.map((item: any, index: number) => (
              <div key={item.name} className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-serif italic text-brand-ink/60 tabular-nums">{formatPrice(item.value)}</span>
              </div>
            ))}
          </div>
        </motion.div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4">
                    <ValuationCell amountUsd={product.price} className="inline-block" />
